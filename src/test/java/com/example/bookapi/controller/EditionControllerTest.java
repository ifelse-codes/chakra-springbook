...
class EditionControllerTest {
    ...
    void createAndFetchEdition() throws Exception {
        mockMvc.perform(post("/api/editions")
                        ...
                        .content("{\"title\":\"Spring in Action\",\"editionNumber\":6,\"publicationYear\":2022}"))
    ...
    void updateEdition() throws Exception {
        Edition saved = editionRepository.save(new Edition("Old Edition", 1, 2018));

        mockMvc.perform(put("/api/editions/{id}", saved.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"title\":\"New Edition\",\"editionNumber\":2,\"publicationYear\":2024}"))
                .andExpect(status().isOk())
    ...
    void deleteEdition() throws Exception {
        Edition saved = editionRepository.save(new Edition("Delete Me", 3, 2020));
